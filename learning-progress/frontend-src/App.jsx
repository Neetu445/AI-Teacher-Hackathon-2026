...
{view === "profile" && profile && <Profile p={profile} history={history} />}
...
function Profile({ p, history = [] }) {
  const scores = history.length ? history : p.scores || [];

  const average = scores.length
    ? Math.round(
        scores.reduce((sum, x) => sum + (x.score || 0), 0) / scores.length
      )
    : 0;

  return (
    <>
      <p className="chip">Learning progress</p>
      <h1>{p.name}'s Progress</h1>
      ...
      <div className="card">
        ...
        <p>
          <strong>Language:</strong> {p.language}
        </p>
        <p>
      ...
      <div className="card" style={{ marginTop: 8 }}>
        ...
        <p className="muted">
          Average score across {scores.length} completed lesson
          {scores.length === 1 ? "" : "s"}.
        </p>
      </div>
      ...
      <div className="card" style={{ marginTop: 8 }}>
        ...
        <p>{p.topics?.join(", ") || "No topics completed yet."}</p>
      </div>

      <div className="card" style={{ marginTop: 8 }}>
        <h3>Strong areas</h3>
        <p>{p.strong?.join(", ") || "No strong areas recorded yet."}</p>
      </div>

      <div className="card" style={{ marginTop: 8 }}>
        <h3>Needs practice</h3>
        <p>{p.weak?.join(", ") || "No areas needing practice recorded yet."}</p>
      </div>

      <div className="card" style={{ marginTop: 8 }}>
        <h3>Misconceptions</h3>
        <p>
          {p.misconceptions?.join(", ") || "No misconceptions recorded."}
        </p>
      </div>
